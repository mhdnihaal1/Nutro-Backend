...
class UserUsecase {
    ...
    async checkExist(email: string) {
      ...
      async verifyOtp(email: string, otp: number) {
        ...
        return { status: 200, message: "User created successfully", data: userData };
      }
      ...
      async Login( email: string,  password: string) {
        ...
        if (user) {
          ...
          if (passwordMatch) {
            ...
            token = this.JwtToken.generateToken(data._id.toString(), "user");
      ...
      async manageAddToCart(data:any) {
        ...
        for (const item of data) {
          const existingItemIndex = cart.items.findIndex((cartItem:any )=>{
           return cartItem.clothItemId.toString() == item.id &&  cartItem.service === item.service
        });
          ...
          if (existingItemIndex > -1) {
            ...
            cart.items.push({ clothItemId: itemIdObj, quantity: item.quantity, service :item.service })
          }
        }
      ...
      async   quantityChange(cartItemId:string,action:string,User:string) {
        const updatecart = await this.UserRepository.quantityChange(cartItemId,action,User);
        return updatecart; 
      }
      ...
      async   createUser(userId:string,nearBy:string ,street:string ,city:string , state:string , postalCode:string) {
        const createuser = await this.UserRepository.createUser(userId,nearBy ,street ,city , state , postalCode);
        return createuser;
      }
      ...
      async editAddress(userId:string,_id:string,nearBy:string,street:string,city:string,state:string,postalCode:string) {
        const address = await this.UserRepository.editAddress(userId,_id,nearBy,street,city,state,postalCode);
        return address; 
      }


      async placeOrder(userId:string,selectedAddress:string,cartTotal:string,selectedMode:string,paymentMethod:string) {
        ...
        const address = await this.UserRepository.findaddress(userId,selectedAddress);
        if(!address){
 ...
 const combineAllToOrder = await this.UserRepository.CombineAllToOrder(user,cartItems,address,agent,cartTotal,selectedMode,paymentMethod);
      ...
      async changePassword(UserId: string, currentPassword: string, newPassword: string) {
        ...
        const isMatch = await this.EncryptPassword.compare(currentPassword, user.password);
      ...
      async handleCancelorder(_id: string ) {
        ...
        this.generateEmail.sendCancelMail(order.userId.email);
      ...
      async addConcern(id: string,subject:string,summary:string ) {
        ...
        this.generateEmail.sendMailtoadmin(admin?.email);
      ...
      async sendEmail(email: string,otp:string ) {
        ...
        this.generateEmail.forgetPassword(email,otp);
